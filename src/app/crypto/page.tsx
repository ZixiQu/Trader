...
export default function CryptoPage() {
	const [activeCrypto, setActiveCrypto] = useState<CryptoSymbol | 'ALL'>('ALL');
	const [range, setRange] = useState<'1d' | '1mo' | '3mo'>('1d');
	const router = useRouter();
	const rawData = useCryptoData(activeCrypto, range);

	const combinedData = useMemo(() => mergeStockData(rawData), [rawData]);

	const activeSymbols = activeCrypto === 'ALL' ? CRYPTO : [activeCrypto];
