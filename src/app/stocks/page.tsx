    ...
    return (
        <div className="w-full flex flex-row">
                ...
                <Card className="py-4 sm:py-0 w-full">
                    <CardHeader className="flex flex-col items-stretch border-b !p-0 sm:flex-row">
                        <div className="flex flex-1 flex-col justify-center gap-1 px-6 pb-3 sm:pb-0">
                            <Select value={activeStock} onValueChange={v => setActiveStock(v as any)}>
                                <SelectTrigger className="h-auto px-0 py-0 border-none shadow-none focus:ring-0 focus:ring-offset-0">
                                    <CardTitle className="cursor-pointer">
                                        <SelectValue placeholder="Select Stock" defaultValue={activeStock} />
                                    </CardTitle>
                                </SelectTrigger>
                    ...
                    <CardContent className="px-2 sm:p-6 space-y-4">
                        <ChartContainer config={chartConfig} className="">
                            <LineChart data={combinedData} width={undefined} margin={{ left: 12, right: 12 }}>
                                ...
                                {activeSymbols.map(symbol => (
                                    <YAxis key={symbol} yAxisId={symbol} domain={['dataMin', 'dataMax']} hide />
                                ))}
                                ...
                                {activeSymbols.map((symbol, i) => (
                                    <Line key={symbol} dataKey={symbol} yAxisId={symbol} name={symbol} type="monotone" stroke={`var(--chart-${(i % 6) + 1})`} strokeWidth={2} dot={false} />
                                ))}
                            </LineChart>
                        </ChartContainer>
                    </CardContent>
            ...
            <div className="w-1/3 flex items-center justify-center flex flex-col px-3">
                <Holdings />
                <Button className="mt-5 w-50 text-base">Go Trading</Button>
            </div>
        </div>
    );
}
